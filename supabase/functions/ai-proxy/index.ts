...
// Limites de payload — proteção de custo (Gemini/OpenAI cobram por token de imagem).
// Client envia no máx 4 imagens (CaptureStep MAX_IMAGES) como image/jpeg (canvas 1280px)
// ou application/pdf cru; caps server-side dão headroom sem permitir abuso.
const MAX_IMAGES = 5;
const MAX_IMAGE_B64_CHARS = 11_000_000; // ~8 MB binário
...
// ── Validação de payload (400 com mensagem de campo; nunca 500 opaco) ─────────

function validateImages(raw: unknown): { ok: true; images: ImageInput[] } | { ok: false; error: string } {
  if (!Array.isArray(raw) || raw.length === 0) {
    return { ok: false, error: "'images' deve ser um array com ao menos 1 item." };
  }
  if (raw.length > MAX_IMAGES) {
    return { ok: false, error: `Máximo de ${MAX_IMAGES} imagens por requisição.` };
  }
  for (const img of raw) {
    if (typeof img?.base64 !== "string" || typeof img?.mimeType !== "string") {
      return { ok: false, error: "Cada imagem deve ter 'base64' e 'mimeType' (strings)." };
    }
    if (!ALLOWED_MIME.has(img.mimeType)) {
      return { ok: false, error: `mimeType não suportado: ${img.mimeType}.` };
    }
    if (img.base64.length > MAX_IMAGE_B64_CHARS) {
      return { ok: false, error: "Arquivo excede o tamanho máximo de 8 MB." };
    }
  }
  return { ok: true, images: raw as ImageInput[] };
}

function validateText(raw: unknown, field: string): { ok: true; text: string } | { ok: false; error: string } {
  if (typeof raw !== "string" || raw.trim().length === 0) {
    return { ok: false, error: `'${field}' deve ser uma string não vazia.` };
  }
  if (raw.length > MAX_TEXT_CHARS) {
    return { ok: false, error: `'${field}' excede ${MAX_TEXT_CHARS} caracteres.` };
  }
...
// Client admin cacheado no módulo — evita recriar conexão a cada chamada de log
...
function logRouting(
  ...
  getAdmin().from("ai_routing_log").insert({
  ...
  try {
    ...
    try {
      ...
      if (type === "receipt_images" || type === "material_images") {
        ...
        const v = validateText(body.transcript, "transcript");
        if (!v.ok) return new Response(JSON.stringify({ error: v.error }), { status: 400, headers: resHeaders });
        const t = v.text;
      ...
      // Detalhe completo apenas em telemetria + logs internos — nunca exposto ao client
      // (mensagens de provider revelam infraestrutura, quotas e estrutura interna).
      const msg = err instanceof Error ? err.message : String(err);
      const latencyMs = Date.now() - startTime;
      logRouting(type as string, "gemini_1", false, latencyMs, false, msg.slice(0, 100));
      console.error(`[ai-proxy] ${String(type)} falhou: ${msg.slice(0, 500)}`);
      return new Response(
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`[ai-proxy] erro não tratado: ${msg.slice(0, 500)}`);
    return new Response(
