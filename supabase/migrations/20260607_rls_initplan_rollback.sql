-- Rollback: fix_rls_initplan_all_policies
...
ALTER POLICY "template_items_all" ON public.checklist_template_items
  USING ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Admin'::user_role, 'Master'::user_role]))))));
...
ALTER POLICY "checklist_templates_insert" ON public.checklist_templates
  WITH CHECK ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Admin'::user_role, 'Master'::user_role, 'Gestor'::user_role]))))));
...
ALTER POLICY "client_locations_delete" ON public.client_locations
  USING ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Admin'::user_role, 'Master'::user_role, 'Gestor'::user_role, 'Supervisor'::user_role]))))));
...
ALTER POLICY "Permitir MODIFICAR clients apenas para Gestão" ON public.clients
  USING ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Admin'::user_role, 'Gestor'::user_role, 'Master'::user_role, 'Supervisor'::user_role]))))))
  WITH CHECK ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Admin'::user_role, 'Gestor'::user_role, 'Master'::user_role, 'Supervisor'::user_role]))))));
...
ALTER POLICY "equipments_managers_all" ON public.equipments
  USING ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role, 'Financeiro'::user_role]))))))
  WITH CHECK ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role, 'Financeiro'::user_role]))))));
...
ALTER POLICY "Managers can manage all requests" ON public.material_requests
  USING ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Comprador'::user_role, 'Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role]))))));
...
ALTER POLICY "Users can view their own requests and managers can view all" ON public.material_requests
  USING (((tech_id = auth.uid()) OR (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Comprador'::user_role, 'Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role])))))));
...
ALTER POLICY "comprador_update_all" ON public.material_requests
  USING ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Comprador'::user_role, 'Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role]))))))
  WITH CHECK ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Comprador'::user_role, 'Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role]))))));
...
ALTER POLICY "notifications_managers_all" ON public.notifications
  USING (((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role, 'Financeiro'::user_role]))))) AND (team_id = get_caller_team_id())))
  WITH CHECK (((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role, 'Financeiro'::user_role]))))) AND (team_id = get_caller_team_id())));
...
ALTER POLICY "orcamento_itens_delete" ON public.orcamento_itens
  USING ((EXISTS ( SELECT 1
   FROM orcamentos o
  WHERE ((o.id = orcamento_itens.orcamento_id) AND ((o.technician_id = auth.uid()) OR (EXISTS ( SELECT 1
           FROM users
          WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role]))))))))));

ALTER POLICY "orcamento_itens_insert" ON public.orcamento_itens
  WITH CHECK ((EXISTS ( SELECT 1
   FROM orcamentos o
  WHERE ((o.id = orcamento_itens.orcamento_id) AND (o.technician_id = auth.uid())))));
...
ALTER POLICY "orcamento_itens_select" ON public.orcamento_itens
  ...
  WHERE ((o.id = orcamento_itens.orcamento_id) AND ((o.technician_id = auth.uid()) OR (EXISTS ( SELECT 1
           FROM users
          WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role]))))))))));

ALTER POLICY "orcamentos_delete" ON public.orcamentos
  USING ((((technician_id = auth.uid()) AND (status = 'rascunho'::orcamento_status)) OR (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Admin'::user_role, 'Master'::user_role])))))));
...
ALTER POLICY "orcamentos_select" ON public.orcamentos
  USING (((technician_id = auth.uid()) OR (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role])))))));
...
ALTER POLICY "reimbursement_history_select" ON public.reimbursement_history
  USING (((reimbursement_id IN ( SELECT reimbursements.id
   FROM reimbursements
  WHERE (reimbursements.user_id = auth.uid()))) OR (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Supervisor'::user_role, 'Financeiro'::user_role])))))));
...
ALTER POLICY "report_attachments_insert" ON public.report_attachments
  ...
  WHERE ((r.id = report_attachments.report_id) AND ((r.technician_id = auth.uid()) OR (EXISTS ( SELECT 1
           FROM users
          WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Supervisor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Financeiro'::user_role])))))))))));
...
ALTER POLICY "checklist_items_insert" ON public.report_checklist_items
  WITH CHECK ((EXISTS ( SELECT 1
   FROM service_reports r
  WHERE ((r.id = report_checklist_items.report_id) AND (r.technician_id = auth.uid())))));

ALTER POLICY "checklist_items_select" ON public.report_checklist_items
  USING ((EXISTS ( SELECT 1
   FROM service_reports r
  WHERE ((r.id = report_checklist_items.report_id) AND ((r.technician_id = auth.uid()) OR (EXISTS ( SELECT 1
           FROM users
          WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Supervisor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Financeiro'::user_role]))))))))));
...
ALTER POLICY "report_signatures_insert" ON public.report_signatures
  WITH CHECK (((EXISTS ( SELECT 1
   FROM service_reports r
  WHERE ((r.id = report_signatures.report_id) AND (r.technician_id = auth.uid())))) OR (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Admin'::user_role, 'Master'::user_role])))))));
...
ALTER POLICY "report_signatures_select" ON public.report_signatures
  ...
  WHERE ((r.id = report_signatures.report_id) AND ((r.technician_id = auth.uid()) OR (EXISTS ( SELECT 1
           FROM users
          WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Supervisor'::user_role, 'Admin'::user_role, 'Master'::user_role]))))))))));

ALTER POLICY "report_history_insert" ON public.report_status_history
  WITH CHECK (((changed_by = auth.uid()) AND (EXISTS ( SELECT 1
   FROM service_reports r
  WHERE ((r.id = report_status_history.report_id) AND ((r.technician_id = auth.uid()) OR (EXISTS ( SELECT 1
           FROM users
          WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Supervisor'::user_role, 'Admin'::user_role, 'Master'::user_role, 'Financeiro'::user_role])))))))))));
...
ALTER POLICY "reports_delete" ON public.service_reports
  USING (((status = 'draft'::report_status) AND ((technician_id = auth.uid()) OR (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Admin'::user_role, 'Master'::user_role]))))))));
...
ALTER POLICY "reports_update" ON public.service_reports
  USING ((((technician_id = auth.uid()) AND (status = ANY (ARRAY['draft'::report_status, 'returned'::report_status]))) OR (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Gestor'::user_role, 'Supervisor'::user_role, 'Admin'::user_role, 'Master'::user_role])))))));
...
ALTER POLICY "admin_all" ON public.service_types
  USING ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Master'::user_role, 'Admin'::user_role, 'Supervisor'::user_role]))))))
  WITH CHECK ((EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND (users.role = ANY (ARRAY['Master'::user_role, 'Admin'::user_role, 'Supervisor'::user_role]))))));
...
ALTER POLICY "users_select" ON public.users
  USING (((id = auth.uid()) OR (get_auth_role() = ANY (ARRAY['Comprador'::text, 'Gestor'::text, 'Admin'::text, 'Master'::text, 'Supervisor'::text]))));

ALTER POLICY "users_update" ON public.users
  USING (((id = auth.uid()) OR (get_auth_role() = ANY (ARRAY['Admin'::text, 'Gestor'::text, 'Master'::text]))));
