...
Isso mantém o hook `Stop` satisfeito (o histórico no repo está sincronizado com o HEAD). O nome do arquivo `last-vault-sync` é legado — hoje significa apenas "histórico do repo sincronizado com este HEAD"; mantido com esse nome para não precisar editar o hook em `settings.json`.
