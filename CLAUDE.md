...
| **Read replica** | Rotear queries de leitura pesada (relatórios, dashboard, listagens paginadas) para réplica Supabase. | Somente no plano Pro+ e somente se CPU do primary saturar após os fixes de initplan; não antecipar || **A11y — 4 violações Axe pré-existentes** | Detectadas pelo CI Axe (audita produção): `button-name` em `/agenda`, `/admin/checklist-templates`, `/admin/os-imports` + `nested-interactive` em `/admin/checklist-templates`. Botões de ícone sem `aria-label` + um nested-interactive — mesmo padrão já resolvido no sprint de a11y anterior. Atualizar `audit/baseline-axe.json` se forem aceitas. | Próximo sprint de a11y; fecha o check vermelho do CI por PR |
