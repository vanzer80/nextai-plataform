...
| **SoC fase 2 — services restantes** | Telas com service existente mas acesso `supabase` inline parcial: `orcamentos/*`, `reports/*`, `agenda`, `portal/ClientPortal`. Extrair o inline para os services já existentes (mesmo lift verbatim dos PRs #4/#5/#6). `auth/Login` fica de fora (`supabase.auth` legítimo); `dashboard/useDashboardData.ts` + `dashboardPreferencesService.ts` são a própria camada de dados. | Sprint dedicado a SoC fase 2; baixo risco, alto volume |
