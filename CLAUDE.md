...
**SoC fase 2 — concluída (2026-06-13):** extraídos os acessos `supabase` inline restantes na UI. Novos services: `osImportService` (Edge Function `os-import-processor`) e `notificationService` (realtime + push + mark-read). Funções novas: `reportService.{reserveOsNumber, fetchReportDetail, signReportMediaUrls}`, `tenantManagementService.{fetchOwnTenantCommercial, updateOwnTenantCommercial}`, `employeeService.{uploadCertificationFile, getEmployeeReports}`. Telas tocadas: `reports/Step1Identification`, `reports/ImportOsDialog`, `useReportDetail`, `admin/CompanyProfile`, `rh/EmployeeDetail`, `AppLayout`, `usePushNotification` — o doc anterior apontava `orcamentos/*`/`agenda`/`portal`, mas estes **já estavam limpos** (extraídos pelo trabalho concorrente). **UI 100% via service** — exceto `auth/Login` (`supabase.auth` legítimo) e `dashboard/useDashboardData.ts` + `dashboardPreferencesService.ts` (a própria camada de dados, sob `pages/`). Gate: tsc EXIT:0 · 163/163 vitest · review adversarial 6/6 limpa.
...
`api-gateway` v5 (valida X-API-Key SHA-256 · rate limit 1000 req/hr · RFC 7807 · cursor pagination composto · idempotency · `api_access_log` · **deploy com `verify_jwt=false`** — armadilha #72 · **rate limiter fail-open + try/catch externo** — armadilha #60 corrigida 2026-06-12 · clients expõe `email`/`phone` mas mapeia p/ `contato_email`/`contato_telefone` — armadilha #71 · endpoints: GET/POST/PATCH orders · GET reimbursements · GET/POST/PATCH clients · GET quotes)  
`os-import-processor` v9 (X-API-Key + Bearer JWT · scope orders:write · mode json/pdf · **template registry** Decathlon + **IA híbrida** Gemini→OpenAI · **modelo gemini-2.5-flash + thinkingBudget:0** (2.0-flash zerado — mesma migração do ai-proxy v15) · per-field confidence scores · resolução client/técnico · `os_import_log` · `import_confidence` em service_reports · **v8: fotos embutidas no PDF extraídas server-side** via `document/image-extractor.ts` (parsing de XObjects /DCTDecode sem deps, filtros anti-logo: dimensão ≥200px, aspect ≤4, dedupe FNV-1a) → upload em `reports_media/{team}/reports/{os}/attachments/` + INSERT em `report_attachments` (caption "Importada do PDF de origem"; `uploaded_by` NULL em imports via API key — migration `20260612`) · resposta inclui `photos_from_pdf` · **v9: datas Decathlon EN-US (M/D/YYYY)** — componente >12 desambigua; ambígua → locale do template (M/D, confidence 0.55), template prevalece sobre a IA no merge da service_date + `requires_review` forçado (armadilha #69))  
...
67. **Fotos de OS importada: bucket `reports` NÃO existe** → o os-import-processor ≤v7 subia fotos para o bucket `reports` (inexistente) e não inseria linha em `report_attachments` — todo upload falhava silenciosamente e nada aparecia na tela da OS. O app lê anexos da tabela `report_attachments` + signed URLs do bucket `reports_media` no caminho `{teamId}/reports/{reportId}/attachments/...` (convenção do reportService.ts). Corrigido na v8. Qualquer novo fluxo que anexe arquivo a OS deve usar tabela + bucket + caminho idênticos.
68. **PDFs "Microsoft Print to PDF" não têm camada de texto** → o texto vira contornos vetoriais (ops `constructPath`/`setFillRGBColor`, zero text items). `extractText` (unpdf/pdfjs) retorna vazio e o template registry nunca casa — o documento segue para o Gemini multimodal (inline PDF), que faz OCR. O fallback OpenAI é texto-only e não cobre esse caso: sem chave Gemini funcional, PDFs vetoriais falham. Diagnóstico: producer "Microsoft: Print To PDF" nos metadados do PDF.
69. **Datas dos PDFs Decathlon são EN-US (M/D/YYYY)** → "4/8/2026 2:31 PM" = 8 de abril, não 4 de agosto (rodapé "Saturday, June 6, 2026" confirma o locale). O parser `decathlon-chamado.ts` usa heurística: componente > 12 desambigua em qualquer ordem; ambos ≤ 12 → locale do template (M/D) com confidence 0.55. O pipeline então impede a IA de sobrescrever a data no merge (a confidence fixa 0.78 da IA mascararia a ambiguidade) e força `requires_review`. Todo novo parser de template deve definir explicitamente como interpreta datas numéricas — nunca assumir DD/MM.
71. **Tabela `clients` não tem colunas `email`/`phone`** → os campos de contato são `contato_email` e `contato_telefone` (além de `cnpj`, `logradouro`, `numero`, `complemento`, `bairro`, `cep`, `cidade`, `estado` char(2), `contato_nome`, `observacoes`, `team_id`). Qualquer `.select('...email, phone...')` ou `.insert({ email, phone })` direto na tabela retorna erro do PostgREST → 500. A api-gateway expõe o contrato público como `email`/`phone` e faz a ponte: read com alias (`email:contato_email, phone:contato_telefone`), write com mapeamento (`mapClientCols`). Esse bug ficou 100% mascarado pela armadilha #60 (tudo morria no rate limiter antes de chegar no SELECT) até a v5.
72. **api-gateway (API pública) deve ser deployada com `verify_jwt=false`** → a auth é o header `X-API-Key` (hash SHA-256 + scopes + rate limit + access log dentro da função), não um JWT Supabase. Com `verify_jwt=true` (default), o gateway da plataforma rejeita com `UNAUTHORIZED_NO_AUTH_HEADER` antes de chegar na função — o ERP cliente que manda só `X-API-Key` (como a doc descreve) leva 401 e a API fica inutilizável. Todo redeploy via MCP/CLI **tem que** passar `verify_jwt=false`. CORS allow-headers inclui `authorization` (para clientes browser que opcionalmente mandem Bearer). Estado atual: v5, verify_jwt=false.
...
| **A11y — 10 violações Axe (app-wide, pré-existentes)** | O harness de auditoria foi corrigido (`waitForAppReady` em vez de `waitForTimeout(3000)` que capturava o skeleton de cold-start) e revelou a dívida REAL — as "4" anteriores eram subcontagem de harness quebrado. **`button-name`** (botões de ícone/DropdownMenu de lista sem `aria-label`, 17 nós) em `/orcamentos`, `/equipments`, `/reimbursements`, `/cp/payables`, `/rh/employees`, `/dp/vacation`, `/dp/timerecords`, `/knowledge`, `/admin/os-imports`; **`label`** (input sem label, 2 nós) em `/reimbursements`. `/agenda` e `/admin/checklist-templates` já corrigidos (s68). Baseline=0, então o CI Axe falha em PR. Run de referência: `actions/runs/27499196069`. | Sprint dedicado de a11y: sweep de `aria-label` nos botões de ícone das listas + label do form de reembolsos; re-disparar `workflow_dispatch` do a11y-audit até verde |
