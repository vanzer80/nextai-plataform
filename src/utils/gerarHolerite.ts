...
export function gerarHolerite(
  ...
  doc.text('HOLERITE / RECIBO DE PAGAMENTO', W / 2, y, { align: 'center' });
  y += 7;
  ...
  y += 5;
  doc.text(`Competência: ${fmtCompetencia(period.competencia)}`, W / 2, y, { align: 'center' });
  y += 8;
  ...
  y = Math.max(tableEndY, (doc as unknown as { lastAutoTable: { finalY: number } }).lastAutoTable.finalY) + 6;
  ...
  doc.save(`holerite_${emp?.matricula ?? emp?.full_name?.split(' ')[0] ?? 'colaborador'}_${period.competencia}.pdf`);
}
